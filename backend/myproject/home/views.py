...
def generate_pdf_report(request):
   ...
   # Generate a random graph
    ...
    graph_buffer.seek(0)  # Rewind the buffer to the beginning

    # Use the BytesIO object to create an Image
    graph_image = Image(graph_buffer)
    graph_image.width = 5 * inch  # Set the desired width
    graph_image.height = 3 * inch  # Set the desired height
...
def create_graph(request):
    ...
    # print('hi',safe_test_call(FrequencyTest.monobit_test, 'Frequency Monobit'))
    # result = FrequencyTest.monobit_test(binary_data)
